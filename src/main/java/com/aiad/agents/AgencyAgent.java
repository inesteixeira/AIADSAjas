...
public class AgencyAgent extends Agent {
	...
	protected void setup() {
		...
		try {
			...
			brokers = new sajas.core.AID[result.length];
			...
			case 2:
				...
				if (cfpBroker != null) {
					...
					if (cfpBroker.getPerformative() == ACLMessage.PROPOSE) {
						...
						double brokerComission = Double.parseDouble(cfpBroker.getContent().split("\\|")[0]);
						double brokerRating = Double.parseDouble(cfpBroker.getContent().split("\\|")[1]);
						double brokerWorth = (1/brokerRating)*0.4 + 0.6*brokerComission;
						System.out.println("BROKER WORTH " + cfpBroker.getSender().getName()  + ": " + brokerWorth);
						if (bestBroker == null || brokerWorth < bestComission) {
							bestComission = brokerWorth;
