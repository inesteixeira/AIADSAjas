...
public class BrokerAgent extends Agent {
    ...
    public BrokerAgent(String name, String type, String commission, String applies, String max) {
        this.name = name;

        if(typeOfInsurance == null)
            typeOfInsurance = new HashMap<String, Integer>();

        for (String insurance : Arrays.asList((type).split("\\|"))) {
            typeOfInsurance.put(insurance, 0);
        }
        this.comission = commission;
        this.appliesDiscount = Boolean.parseBoolean(applies);
        this.maxDiscount = Integer.parseInt(max);
    }
                ...
                case 0:
                    if (msg != null) {
                        ...
                        if (typeOfInsurance.containsKey(clientTypeOfInsurance)) {
                            ...
                            reply.setContent(comission + "| " +  rating);
                        } else {
                            reply.setPerformative(ACLMessage.REFUSE);
                            System.out.println("Broker " + name + " sent a REFUSE to Agency. Do not sells that insurance.");
                        }
            ...
            if (activeBroker) {
                switch (step) {
                    ...
                    case 1:
                        ...
                        if (reply != null) {
                            if (reply.getPerformative() == ACLMessage.PROPOSE && appliesDiscount) {
                                ...
                                step = 2;
                                    ...
                                    ACLMessage newReply = reply.createReply();
                                    //newReply.setConversationId("fromBroker2");
                                    newReply.setPerformative(ACLMessage.PROPOSE);
                                    newReply.setContent(String.valueOf(priceInsurance));
                                    newReply.addReceiver(client);
                                    myAgent.send(newReply);
                                    System.out.println("Broker " + name + " does not negotiates. Sending same price offer of " + priceInsurance + ".");
                                    step = 2;
